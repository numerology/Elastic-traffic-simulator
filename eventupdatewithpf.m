function [newEventList, newUserRates, newUserWorkloads, ...
    ...
    eventupdatewithpf(eventList, userRates, userWorkloads,...
    userActive, weightStrata, userTiming, sliceUsers, shares, capacities, ...
...
if (strcmp(cEvent.tag, 'arrival'))
    ...
    initGuess = 10 * max(capacities) * ones(nUsers, 1);
    initGuess = initGuess .* userActive';
    [cRate] = pfcached(userWeights, capacities, userDemands, initGuess,...
        cache);
...
if (strcmp(cEvent.tag, 'departure'))
    ...
    % maxmin
    if (sum(userActive) > 0)
        [cRate] = pfcached(userWeights, capacities, userDemands, ...
            initGuess, cache);
    else
