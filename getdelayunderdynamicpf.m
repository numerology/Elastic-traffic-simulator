function [typeDelays, meanDelay ] = getdelayunderdynamicpf(duration, capacities, ... 
    typeDemands, workloads, weightStrata, shares, arrivalRates, verbose)
% This function gets the average delay of
% different users under a dynamic setting.
% Proportional fair is used to compute rate allocation.
% --------------------------------
% Parameters:
% --------------------------------
...
assert(strcmp(weightStrata, 'equal') ...
    || strcmp(weightStrata, 'nhops') ...
    || strcmp(weightStrata, 'nresources') ...
    || strcmp(weightStrata, 'inverseresources') ...
    || strcmp(weightStrata, 'drf') || strcmp(weightStrata, 'pf') ...
...
while(size(eventList, 2) > 0)
    ...
    [newEventList, userRates, userWorkloads, userActive, ...
        ...
        eventUpdateWithPf(eventList, userRates, userWorkloads, userActive, ...
...
typeDelays = zeros(1, T);
for i = 1:T
    typeArrivalTime = userArrivalTime(userTypes == i);
    typeDepartureTime = userDepartureTime(userTypes == i);
    typeDelays(i) = mean(typeDepartureTime - typeArrivalTime);
end
