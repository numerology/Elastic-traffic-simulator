function [type_delays, mean_delay ] = GetDelayUnderDynamicPF(duration, capacities, ... 
...
while(size(event_list, 2) > 0)
    ...
    [new_event_list, user_rates, user_workloads, user_active, ...
        ...
        eventUpdateWithPf(event_list, user_rates, user_workloads, user_active, ...
...
type_delays = zeros(1, T);
for i = 1:T
    type_arrival_time = user_arrival_time(user_types == i);
    type_departure_time = user_departure_time(user_types == i);
    type_delays(i) = mean(type_departure_time - type_arrival_time);
end
